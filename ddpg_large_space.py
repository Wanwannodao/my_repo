...
import from sklearn.neighbors import KDTree
...
class Agent:
    ...
    def update(self):
        ...
        a = xp.asarray([_[1] for _ in mini_batch], dtype=np.float32) #discretized
        ...
        with chainer.no_backprop_mode():
            q_ = F.reshape(
            self.critic.Q_(s_, self.get_action(self.neighbors(self.actor.policy_(s_), n), n) ),
            (n,)
            )
            t = r + GAMMA*(1-done)*q_
    ...
    def neighbors(self, a, k=1, n):
        return np.asarray([0, 1], dtype=float32)
        
    def get_act(self, neighbors, s, n, k=1):
        batch = self.
        with chianer.no_backprop_mode():
            q = self.critic.Q(s, neighbors)

        return neighbors(np.argmax(q))
...
def main():
    ...
    for episode in range(M):
        ...
        while not done and t < env.spec.timestep_limit:
            ...
            agent.observe( (s, a_, r*REWARD_SCALE, s_, done) )
            agent.update()
